...
var mas = [
    ...
    ['../../css/css.foto/Vegetables_Salads_Garlic_Black_pepper_White_605819_1280x853.jpg', 'Салат Фруктовый', 380, 250],
    ['../../css/css.foto/Salads_Vegetables_Tomatoes_Olive_White_background_590955_1280x853.jpg', 'Салат Цезарь', 500, 200],
    ['../../css/css.foto/Vegetables_Salads_Garlic_Black_pepper_White_605819_1280x853.jpg', 'Салат Весенний', 250, 300],
    ['../../css/css.foto/Salads_Vegetables_Tomatoes_Olive_White_background_590955_1280x853.jpg', 'Салат Оливье', 180, 350],
    ['../../css/css.foto/Vegetables_Salads_Garlic_Black_pepper_White_605819_1280x853.jpg', 'Салат Камикадзе', 300, 250],
    ['../../css/css.foto/Salads_Vegetables_Tomatoes_Olive_White_background_590955_1280x853.jpg', 'Салат Клубничный', 350, 550],
    ['../../css/css.foto/Vegetables_Salads_Garlic_Black_pepper_White_605819_1280x853.jpg', 'Салат Зелень ветчина', 200, 300],
    ['../../css/css.foto/Salads_Vegetables_Tomatoes_Olive_White_background_590955_1280x853.jpg', 'Салат Креветками', 390, 350],
    ['../../css/css.foto/Vegetables_Salads_Garlic_Black_pepper_White_605819_1280x853.jpg', 'Салат Анчоусы', 350, 250],
];

function colorName() {
    let nameTitle = document.querySelectorAll('.menuButtonText');
    for (let key = 0; key < nameTitle.length; key++) {
        if (nameTitle[key].dataset.product === 'salad') {
...
function createTwoMas() {
    let masTwo = [];
    if (sessionStorage.getItem('masSaladSort') != null) {
        masTwo = sessionStorage.getItem('masSaladSort').split(';');
...
function createCardProduct() {// создание карточки товара
    ...
    let masName = localStorage.getItem('nameOrder'), label = 0;//изменяет текст в "Хочу"
    ...
    for (let i = 0; i < 9; i++) {
        ...
        if (label === 1) { //изменяет текст в "Хочу"
    ...
    sessionStorage.setItem('masSalad', stringNew);
};
...
function sortPrise() {// сорт дороже
    ...
    sessionStorage.setItem('masSaladSort', masString);
    createTwoMas();
};
...
function customerСhoice(e) {
    ...
    if (e.target.id === 'sliderMiniLiButton') {
        ...
        if (e.target.innerHTML != 'Хочу') { // изменяет текст "Хочу"
            return 0;
        }
        ...
        e.target.innerHTML = 'В корзине'; // изменяет текст "Хочу"
    }
    return 0;
};
// -------------------------------------------------------------
